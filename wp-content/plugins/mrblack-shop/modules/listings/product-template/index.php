...
if( !class_exists( 'MrBlack_Woo_Listing_Fw_Template_Settings' ) ) {

    class MrBlack_Woo_Listing_Fw_Template_Settings {
            ...
            function woo_cs_fw_template_options( $options ) {
                ...
                # Template Name
                    $custom_tpl_name = array( array(
                        ...
                        'type'  => 'text',
                        'title' => esc_html__('Template', 'mrblack-shop')
                    ) );


                # Default Options
                    $custom_tpl_default_options = apply_filters( 'mrblack_woo_custom_product_template_default_options', array () );
                    if( is_array ( $custom_tpl_default_options ) && !empty ( $custom_tpl_default_options ) ) {
                        array_unshift($custom_tpl_default_options, array (
                            'type'    => 'notice',
                            'class'   => 'info',
                            'content' => esc_html__('Default Options.', 'mrblack-shop')
                        ));
                    }

                # Hover Options
                    $custom_tpl_hover_options = apply_filters( 'mrblack_woo_custom_product_template_hover_options', array () );
                    if( is_array ( $custom_tpl_hover_options ) && !empty ( $custom_tpl_hover_options ) ) {
                        array_unshift($custom_tpl_hover_options, array (
                            'type'    => 'notice',
                            'class'   => 'info',
                            'content' => esc_html__('Hover Options.', 'mrblack-shop')
                        ));
                    }

                # Common Options
                    $custom_tpl_common_options = apply_filters( 'mrblack_woo_custom_product_template_common_options', array () );
                    if( is_array ( $custom_tpl_common_options ) && !empty ( $custom_tpl_common_options ) ) {
                        array_unshift($custom_tpl_common_options, array (
                            'type'    => 'notice',
                            'class'   => 'info',
                            'content' => esc_html__('Common Options.', 'mrblack-shop')
                        ));
                    }

                # Thumb Options
                    $custom_tpl_thumb_options = apply_filters( 'mrblack_woo_custom_product_template_thumb_options', array () );
                    if( is_array ( $custom_tpl_thumb_options ) && !empty ( $custom_tpl_thumb_options ) ) {
                        array_unshift($custom_tpl_thumb_options, array (
                            'type'    => 'notice',
                            'class'   => 'info',
                            'content' => esc_html__('Thumb Options.', 'mrblack-shop')
                        ));
                    }

                # Content Options
                    $custom_tpl_content_options = apply_filters( 'mrblack_woo_custom_product_template_content_options', array () );
                    if( is_array ( $custom_tpl_content_options ) && !empty ( $custom_tpl_content_options ) ) {
                        array_unshift($custom_tpl_content_options, array (
                            'type'    => 'notice',
                            'class'   => 'info',
                            'content' => esc_html__('Content Options.', 'mrblack-shop')
                        ));
                    }
                ...
                # Default & Custom Template Section
                    ...
                    $product_template_section = array (
                        array (
                            ...
                            'title'  => esc_html__('Shop - Product Template', 'mrblack-shop'),
                            'icon'   => 'fa fa-shopping-bag',
                            'fields' => array_merge (
                                apply_filters( 'mrblack_woo_default_product_templates', array () ),
                                array (
                                    array (
                                        'id'              => 'mrblack-woo-product-style-templates',
                                        'type'            => 'group',
                                        'title'           => esc_html__( 'Product Style Templates', 'mrblack-shop' ),
                                        'button_title'    => esc_html__('Add New', 'mrblack-shop'),
                                        'accordion_title' => esc_html__('Add New Template', 'mrblack-shop'),
            ...
            function woo_get_options_params( $options_param, $option_type ) {
                ...
                if( is_array ( $options_param ) && !empty ( $options_param ) ) {
                    foreach ( $options_param as $option_param_key => $option_param ) {
                        ...
                        $option_class_instance = 'mrblack_woo_listing_option_'.$option_param_key;  // Option Class Instance
...
if( !function_exists('mrblack_woo_listing_fw_template_settings') ) {
	function mrblack_woo_listing_fw_template_settings() {
		return MrBlack_Woo_Listing_Fw_Template_Settings::instance();
	}
}

mrblack_woo_listing_fw_template_settings();