<?php
class MrBlack_Widget_Recent_Posts extends WP_Widget {
	#1.constructor
	function __construct() {
		$widget_options = array(
			'classname'   => 'widget_recent_posts',
			'description' => esc_html__('To list out posts', 'mrblack-pro')
		);

        $theme_name =  defined('MRBLACK_THEME_NAME') ? MRBLACK_THEME_NAME : 'MrBlack';
		parent::__construct( false, $theme_name . esc_html__(' Posts','mrblack-pro'), $widget_options );
	}
	...
	function form($instance) {
        ...
        <p>
        	<label for="<?php echo esc_attr($this->get_field_id('title')); ?>">
        		<?php esc_html_e('Title:','mrblack-pro');?>
	    ...
	    <p>
	    	<label for="<?php echo esc_attr($this->get_field_id('_post_categories')); ?>">
	    		<?php esc_html_e('Choose the categories you want to display (multiple selection possible)','mrblack-pro');?>
	    	</label>
            <select class="widefat" id="<?php echo esc_attr($this->get_field_id('_post_categories').'[]');?>" name="<?php echo esc_attr($this->get_field_name('_post_categories').'[]');?>" multiple="multiple">
            	<option value=""><?php esc_html_e("Select",'mrblack-pro');?></option><?php
        ...
        <p>
        	<label for="<?php echo esc_attr($this->get_field_id('_excerpt')); ?>">
        		<?php esc_html_e('Display title only or title &amp; excerpt','mrblack-pro');?>
        	</label>
        ...
        <p>
        	...
        	<?php esc_html_e("Show Image",'mrblack-pro');?>
        </p>

	    <p>
	    	<label for="<?php echo esc_attr($this->get_field_id('_post_count'));?>">
	    		<?php esc_html_e('No.of posts to show:','mrblack-pro');?>
	    	</label>
	...
	function widget($args, $instance) {
		...
		echo mrblack_pro_before_after_widget( $before_widget );

		if( !empty( $title ) ) {

			echo mrblack_pro_widget_title( $before_title . $title . $after_title );
		}
			 ...
			 while( $the_query->have_posts() ):
				...
				echo "<li>";
					if(1 == $_enabled_image):
						...
						$image = ( $image != false)? $image[0] : MRBLACK_PRO_DIR_URL . 'modules/blog/assets/images/post-thumb.jpg';
					...
					echo '<div class="entry-meta">';
						echo '<p>' . get_the_date(get_option('date_format')) . '</p>';
					echo '</div>';
					...
					if($show_title):
						...
						echo "<div class='entry-title'><h4><a href='".get_permalink()."'>{$title}</a></h4></div>";
						echo "<div class='entry-content'>".mrblack_excerpt(10)."</div>";
					endif;

				echo "</li>";
			 endwhile;
			 else:
			 	echo "<li><h4>".esc_html__('No Posts found','mrblack-pro')."</h4></li>";
			 endif;
			 wp_reset_postdata();
	 	echo "</ul></div>";

		echo mrblack_pro_before_after_widget( $after_widget );
	}
}?>