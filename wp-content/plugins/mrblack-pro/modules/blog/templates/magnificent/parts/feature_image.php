        ...
        <div class="entry-thumb-detail-group">
            ...
            <?php if(in_array('title', $archive_post_elements)) :?>
                <div class="entry-title">
                    <h4><?php
                        if( is_sticky( $post_ID ) ) echo '<span class="sticky-post"><i class="wdticon-thumb-tack"></i><span>'.esc_html__('Featured', 'mrblack-pro').'</span></span>'; ?>
                        <a href="<?php echo get_permalink( $post_ID );?>" title="<?php printf(esc_attr__('Permalink to %s','mrblack-pro'), the_title_attribute('echo=0'));?>"><?php the_title();?></a>
                    </h4>
                </div>
            <?php endif; ?>
            <?php if(in_array('content', $archive_post_elements)) :?>
                <?php if( $enable_excerpt_text && $archive_excerpt_length > 0 ) : ?>
                    <div class="entry-body"><?php echo mrblack_excerpt( $archive_excerpt_length );?></div>
                <?php endif; ?>
            <?php endif; ?>
