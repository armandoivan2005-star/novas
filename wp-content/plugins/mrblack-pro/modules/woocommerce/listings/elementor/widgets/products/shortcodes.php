...
if(!function_exists('mrblack_shop_product_listing_html')) {
	function mrblack_shop_product_listing_html($attrs, $content = null) {
		...
		$out = mrblack_shop_products_render_html($attrs);

		return $out;

	}
	add_shortcode( 'mrblack_shop_product_listing', 'mrblack_shop_product_listing_html' );
}

if(!function_exists('mrblack_shop_products_render_html')) {
	function mrblack_shop_products_render_html($settings) {
		...
		if ( isset( $_REQUEST['mrblack_shop_search_submit'] ) && !empty( $_REQUEST['mrblack_shop_search_submit'] ) ) {
			$settings['page_requests'] = $_REQUEST;
		}

		$ps_nonce_verified = false;
		if ( isset( $settings['page_requests']['mrblack_shop_product_search_nonce'] ) && wp_verify_nonce( $settings['page_requests']['mrblack_shop_product_search_nonce'], 'mrblack_shop_product_search' ) ) {
		...
		$type_class_instance = 'mrblack_woo_listing_type_custom';
		if ( function_exists( $type_class_instance ) ) {

			if( $settings['product_style_template'] == -1 ) {
				$type_class_instance()->custom_template = mrblack_customizer_settings('wdt-woo-shop-page-product-style-template' );
			} else {
			...
			if( is_array ( $type_options ) && !empty ( $type_options ) ) {
				foreach ( $type_options as $type_option_key => $type_option ) {
					...
					$option_class_instance = 'mrblack_woo_listing_option_'.$type_option_key;  // Option Class Instance
		...
		$product_ul_class = $product_ul_class . $settings['columns'];
			...
			$classes = apply_filters( 'mrblack_woo_listings_class', array () );
			...
			$output .= '<ul class="products '.$wrapper_class.' '.$classes.' '.esc_attr($product_ul_class).' ">';
				...
				if($settings['enable_carousel'] != 'true') {
					$output .= '<li class="product isotope-grid-sizer"><div class="'.mrblack_woo_loop_column_class($settings['columns']).'"></div></li>';
				}
				...
				ob_start();
					...
					if($ps_nonce_verified) {
						if(isset($settings['page_requests']['mrblack_shop_search_keyword']) && $settings['page_requests']['mrblack_shop_search_keyword'] != '') {
							$args['s'] = $settings['page_requests']['mrblack_shop_search_keyword'];
						}
					}
					...
					if($ps_nonce_verified) {
						...
						$mrblack_shop_search_categories = array ();

						if(isset($settings['page_requests']['mrblack_shop_search_categories']) && !empty($settings['page_requests']['mrblack_shop_search_categories'])) {
							$mrblack_shop_search_categories = $settings['page_requests']['mrblack_shop_search_categories'];
						}

						if(!empty($mrblack_shop_search_categories)) {
							$args['tax_query'][] = array (
														'taxonomy' => 'product_cat',
														'field'    => 'id',
														'terms'    => $mrblack_shop_search_categories,
														'operator' => 'IN'
													);
						}
			...
			if($settings['show_pagination'] == 'true') {
				...
				$output .= mrblack_shop_products_ajax_pagination($max_num_pages, $settings['current_page'], $settings['post_per_page'], 'mrblack_shop_products_ajax_call', 'wdt-products-container', $shortcode_settings);
			}
			...
			if($settings['enable_carousel'] == 'true') {
				...
				$output .= '<div class="wdt-products-pagination-holder">';
					...
					if($settings['carousel_arrowpagination'] == 'true') {
						...
						$output .= '<a href="#" class="wdt-products-arrow-prev wdt-products-arrow-prev-'.esc_attr($settings['module_id']).'">'.esc_html__('Prev', 'mrblack-pro').'</a>';
						$output .= '<a href="#" class="wdt-products-arrow-next wdt-products-arrow-next-'.esc_attr($settings['module_id']).'">'.esc_html__('Next', 'mrblack-pro').'</a>';
						$output .= '</div>';
					}
...
if ( ! function_exists( 'mrblack_shop_products_ajax_call' ) ) {

	function mrblack_shop_products_ajax_call() {
		...
		if(isset($productpagination_nonce) && wp_verify_nonce($productpagination_nonce, 'productpagination_nonce')) {
			...
			$output = mrblack_shop_products_render_html($shortcodeattrs_settings);

			echo mrblack_html_output($output);
	...
	add_action( 'wp_ajax_mrblack_shop_products_ajax_call', 'mrblack_shop_products_ajax_call' );
	add_action( 'wp_ajax_nopriv_mrblack_shop_products_ajax_call', 'mrblack_shop_products_ajax_call' );

}

// Product Shortcode - Ajax Pagination

if ( ! function_exists( 'mrblack_shop_products_ajax_pagination' ) ) {

	function mrblack_shop_products_ajax_pagination($max_num_pages, $current_page, $post_per_page, $function_call, $output_div, $shortcode_attrs) {
...
if( ! function_exists( 'mrblack_shop_single_product_nav' ) ) {
	function mrblack_shop_single_product_nav() {
		...
		$output .= '<div class="wdt-single-product-nav">';
			...
			if ( ! empty( $prev ) ) {
				...
				$output .= '<div class="wdt-single-product-nav-btn product-nav-prev">';
					$output .= '<a href="'.esc_url( $prev->get_permalink() ).'">'.esc_html__('Previous product', 'mrblack-pro').'<span class="product-nav-btn-icon"></span></a>';
			...
			$output .= '<a href="'.esc_url($shop_page_url).'" class="wdt-single-product-nav-back-btn"><span>'.esc_html__('Back to products', 'mrblack-pro').'</span></a>';

			if ( ! empty( $next ) ) {

				$output .= '<div class="wdt-single-product-nav-btn product-nav-next">';
					$output .= '<a href="'.esc_url( $next->get_permalink() ).'">'.esc_html__('Next product', 'mrblack-pro').'<span class="product-nav-btn-icon"></span></a>';
					$output .= '<div class="wdt-single-product-nav-intro-wrapper">';
						$output .= '<div class="product-nav-intro">';
							$output .= '<div class="product-nav-intro-image">';
