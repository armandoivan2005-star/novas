...
if( !class_exists( 'MrBlack_Pro_Woo_Sidebar' ) ) {
    class MrBlack_Pro_Woo_Sidebar {

        private static $_instance = null;
        private $global_layout    = '';
        private $global_sidebar   = '';
        private $global_shop_sidebar   = '';
        ...
        function __construct() {
            ...
            $this->global_shop_sidebar = mrblack_customizer_settings('global_shop_sidebar');
        ...
        function frontend() {
            add_filter('mrblack_primary_classes', array( $this, 'primary_classes' ) );
            add_filter('mrblack_secondary_classes', array( $this, 'secondary_classes' ) );
            add_filter('mrblack_active_sidebars', array( $this, 'active_sidebars' ), 20 );
            add_filter('mrblack_woo_loop_column_class', array( $this, 'woo_loop_column_class' ), 10, 2 );
        }

        function primary_classes( $primary_class ) {

            if( is_shop()  ) {
                $settings = get_post_meta( get_option('woocommerce_shop_page_id'), '_mrblack_layout_settings', true );
        ...
        function active_sidebars( $sidebars = array() ) {
            ...
            if( is_product_category() || is_product_tag() ) {
                $sidebars[] = $this->global_shop_sidebar;
                if($this->hide_standard_sidebar) {
                    unset($sidebars[array_search('mrblack-standard-sidebar-1', $sidebars)]);
                }
            }
...
MrBlack_Pro_Woo_Sidebar::instance();