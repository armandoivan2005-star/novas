...
if( !class_exists( 'MrBlackProCustomFont' ) ) {
    class MrBlackProCustomFont {
        ...
        function load_modules() {
            include_once MRBLACK_PRO_DIR_PATH.'modules/custom-font/taxonomy/index.php';
        }

        function load_customizer(){
            add_filter( 'mrblack_customizer_custom_fonts', array( $this, 'fonts_list' ), 20 );
        }
        ...
        function elementor_group( $font_groups ) {
            ...
            if( count( $fonts ) > 0 ) {
                $new_group[ 'mrblack_custom_fonts' ] = esc_html__( 'Custom Font', 'mrblack-pro' );
        ...
        function add_elementor_fonts( $fonts ) {
            ...
            if( count( $fonts ) > 0 ) {
                foreach( $fonts as $key => $name ) {
                    $fonts[$name]   = 'mrblack_custom_fonts';
                }
            }
        ...
        function load_frontend() {
            add_filter( 'mrblack_add_inline_style', array( $this, 'custom_font_style' ) );
        }
            ...
            if( count( $fonts ) > 0 ) {
                ...
                foreach( $fonts as $font => $data ) {
                    ...
                    $css .= '@font-face { font-family:' . esc_attr( $font ) . ';';
                        ...
                        if( isset( $data['ttf']) ) {
                            $attr[] = 'url(' . esc_url( $data['ttf'] ) . ") format('truetype')";
                        }
        ...
        function get_fonts() {
            ...
            $fonts = array();
            $terms = get_terms( 'mrblack_custom_fonts', array( 'hide_empty' => false ) );
        ...
        function get_font_links( $id ) {
            $meta = get_term_meta( $id, '_mrblack_custom_font_options', true );
