...
if (! class_exists ( 'MrBlackProMegaMenuPostType' ) ) {

	class MrBlackProMegaMenuPostType {
		...
		function __construct() {
            ...
            add_action ( 'admin_print_styles', array( $this, 'mrblack_admin_print_styles') );
        	...
        	add_action( 'wp_update_nav_menu_item', array( $this, 'mrblack_update_menu_item'), 10, 2 );
   			...
   			add_filter( 'wp_nav_menu_objects', array( $this, 'mrblack_nav_menu_objects' ),10, 2 );
		...
		function mrblack_register_cpt() {
			...
			$labels = array (
				'name'				 => __( 'Mega Menus', 'mrblack-pro' ),
				'singular_name'		 => __( 'Mega Menu', 'mrblack-pro' ),
				'menu_name'			 => __( 'Mega Menus', 'mrblack-pro' ),
				'add_new'			 => __( 'Add Mega Menu', 'mrblack-pro' ),
				'add_new_item'		 => __( 'Add New Mega Menu', 'mrblack-pro' ),
				'edit'				 => __( 'Edit Mega Menu', 'mrblack-pro' ),
				'edit_item'			 => __( 'Edit Mega Menu', 'mrblack-pro' ),
				'new_item'			 => __( 'New Mega Menu', 'mrblack-pro' ),
				'view'				 => __( 'View Mega Menu', 'mrblack-pro' ),
				'view_item' 		 => __( 'View Mega Menu', 'mrblack-pro' ),
				'search_items' 		 => __( 'Search Mega Menus', 'mrblack-pro' ),
				'not_found' 		 => __( 'No Mega Menus found', 'mrblack-pro' ),
				'not_found_in_trash' => __( 'No Mega Menus found in Trash', 'mrblack-pro' ),
			);
		...
		function mrblack_template_include($template) {
			if ( is_singular( 'wdt_mega_menus' ) ) {
				if ( ! file_exists ( get_stylesheet_directory () . '/single-wdt_mega_menus.php' ) ) {
					$template = MRBLACK_PRO_DIR_PATH . 'post-types/templates/single-wdt_mega_menus.php';
				}
			}
        ...
        function mrblack_admin_print_styles() {
        ...
        function custom_fields( $item_id, $item, $depth, $args ) {
        	...
        	<p class="field-wdt-mega-menu-width description description-thin">
        		<label for="edit-menu-item-wdt-mega-menu-width-<?php echo esc_attr( $item_id ); ?>">
        			<?php esc_html_e( 'Mega Menu Width (px)','mrblack-pro' ); ?><br />
                    ...
                    <span class="description"><?php esc_html_e('Please set Mega Menu Width',  'mrblack-pro'); ?></span>
        		</label>
        	</p>
        	...
        	<p class="field-wdt-mega-menu-position description description-wide">
        		<label for="edit-menu-item-wdt-mega-menu-position-<?php echo esc_attr( $item_id ); ?>">
        			<?php esc_html_e( 'Mega Menu Position','mrblack-pro' ); ?><br />
        			<select id="edit-menu-wdt-mega-menu-position-<?php echo esc_attr( $item_id ); ?>" class="widefat code edit-menu-item-wdt-mega-menu-position" name="menu-item-wdt-mega-menu-position[<?php echo esc_attr( $item_id ); ?>]">
                        <option value="none" <?php selected( $position, '' );?>><?php esc_html_e('None','mrblack-pro');?></option>
                        <option value="left" <?php selected( $position, 'left' );?>><?php esc_html_e('Left','mrblack-pro');?></option>
                        <option value="right" <?php selected( $position, 'right' );?>><?php esc_html_e('Right','mrblack-pro');?></option>
                        <option value="center" <?php selected( $position, 'center' );?>><?php esc_html_e('Top - Center','mrblack-pro');?></option>
        			</select>
                    <span class="description"><?php esc_html_e('Please set Mega Menu Position',  'mrblack-pro'); ?></span>
        		</label>
        	</p><?php
        }
        ...
        function mrblack_nav_menu_objects( $items, $args ) {
            ...
            foreach ( $items as $item ) {
                if( is_a( $args->walker, 'MrBlack_Walker_Nav_Menu' ) || is_a( $args->walker, 'MrBlack_Default_Header_Walker_Nav_Menu' ) ) {
        ...
        function add_classes( $classes, $item, $args, $depth  ) {
        	...
        	if ($item->object == 'wdt_mega_menus' && $depth == 1) {
                ...
                if( is_a( $args->walker, 'MrBlack_Walker_Nav_Menu' ) || $args->theme_location == 'main-menu' || is_a( $args->walker, 'MrBlack_Default_Header_Walker_Nav_Menu' ) ) {
...
MrBlackProMegaMenuPostType::instance();