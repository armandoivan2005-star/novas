...
if( ! function_exists( 'cs_get_path_locate' ) ) {
  function cs_get_path_locate() {
    ...
    $dirname        = wp_normalize_path( plugin_dir_path( __FILE__ ) );
    $plugin_dir     = wp_normalize_path( WP_PLUGIN_DIR . '\\'.WDT_CS_FOLDER_PATH );
    ...
    $directory_uri  = ( $located_plugin ) ? WP_PLUGIN_URL . '/'.WDT_CS_FOLDER_PATH : get_template_directory_uri();
...
if( ! function_exists( 'cs_locate_template' ) ) {
  function cs_locate_template( $template_name ) {
    ...
    $dir_plugin   = wp_normalize_path( WP_PLUGIN_DIR . '\\'.WDT_CS_FOLDER_PATH );
 ...
 *
 */
load_textdomain( 'mrblack-pro', CS_DIR .'/languages/'. cs_get_locale() .'.mo' );