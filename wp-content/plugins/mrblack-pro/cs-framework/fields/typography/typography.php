...
class CSFramework_Option_typography extends CSFramework_Options {
  ...
  public function output() {
    ...
    if( is_object( $google_json ) ) {
      ...
      echo '<select name="'. esc_attr($this->element_name( '[family]' )) .'" class="'. esc_attr($is_chosen) . $chosen_rtl .'cs-typo-family" data-atts="family"'. mrblack_html_output($this->element_attributes()) .'>';

      do_action( 'cs_typography_family', $family_value, $this );

      echo '<optgroup label="'. esc_attr__( 'Web Safe Fonts', 'mrblack-pro' ) .'">';
      foreach ( $websafe_fonts as $websafe_value ) {
        echo '<option value="'. esc_attr($websafe_value) .'" data-variants="'. implode( '|', $default_variants ) .'" data-type="websafe"'. selected( $websafe_value, $family_value, true ) .'>'. $websafe_value .'</option>';
      }
      echo '</optgroup>';

      echo '<optgroup label="'. esc_attr__( 'Google Fonts', 'mrblack-pro' ) .'">';
      foreach ( $googlefonts as $google_key => $google_value ) {
        echo '<option value="'. esc_attr($google_key) .'" data-variants="'. implode( '|', $google_value ) .'" data-type="google"'. selected( $google_key, $family_value, true ) .'>'. $google_key .'</option>';
      }
      echo '</optgroup>';
      ...
      if( ! empty( $is_variant ) ) {
        ...
        foreach ( $variants as $variant ) {
          echo '<option value="'. esc_attr($variant) .'"'. mrblack_html_output($this->checked( $variant_value, $variant, 'selected' )) .'>'. $variant .'</option>';
        }
        echo '</select>';
        echo '</label>';
      ...
      echo esc_html__( 'Error! Can not load json file.', 'mrblack-pro' );
