...
class CSFramework_Option_Sorter extends CSFramework_Options {
  ...
  public function output(){
    ...
    $enabled_title  = ( isset( $this->field['enabled_title'] ) ) ? $this->field['enabled_title'] : esc_html__( 'Enabled Modules', 'mrblack-pro' );
    $disabled_title = ( isset( $this->field['disabled_title'] ) ) ? $this->field['disabled_title'] : esc_html__( 'Disabled Modules', 'mrblack-pro' );
    ...
    if( ! empty( $enabled ) ) {
      foreach( $enabled as $en_id => $en_name ) {
        echo '<li><input type="hidden" name="'. esc_attr($this->element_name( '[enabled]['. $en_id .']' )) .'" value="'. esc_attr($en_name) .'"/><label>'. $en_name .'</label></li>';
      }
    }
    echo '</ul>';
    echo '</div>';
    ...
    if( ! empty( $disabled ) ) {
      foreach( $disabled as $dis_id => $dis_name ) {
        echo '<li><input type="hidden" name="'. esc_attr($this->element_name( '[disabled]['. $dis_id .']' )) .'" value="'. esc_attr($dis_name) .'"/><label>'. $dis_name .'</label></li>';
      }
    }
    echo '</ul>';
    echo '</div>';
