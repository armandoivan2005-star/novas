...
class CSFramework_Shortcode_Manager extends CSFramework_Abstract{
  ...
  public function media_shortcode_button( $editor_id ) {
    ...
    if( ! in_array( $post_type, $this->exclude_post_types ) ) {
      echo '<a href="#" class="button button-primary cs-shortcode" data-editor-id="'. esc_attr($editor_id) .'">'. esc_html__( 'Add Shortcode', 'mrblack-pro' ) .'</a>';
    }
  ...
  ?>
    <div id="cs-shortcode-dialog" class="cs-dialog" title="<?php esc_attr_e( 'Add Shortcode', 'mrblack-pro' ); ?>">
      <div class="cs-dialog-header">
        <select class="<?php echo ( is_rtl() ) ? 'chosen-rtl ' : ''; ?>cs-dialog-select" data-placeholder="<?php esc_attr_e( 'Select a shortcode', 'mrblack-pro' ); ?>">
          <option value=""></option>
          <?php
            foreach ( $this->options as $group ) {
              ...
              foreach ( $group['shortcodes'] as $shortcode ) {
                ...
                echo '<option value="'. esc_attr($shortcode['name']) .'" data-view="'. esc_attr($view) .'">'. $shortcode['title'] .'</option>';
              }
              echo '</optgroup>';
            }
          ?>
      ...
      <div class="cs-insert-button hidden">
        <a href="#" class="button button-primary cs-dialog-insert"><?php esc_html_e( 'Insert Shortcode', 'mrblack-pro' ); ?></a>
      </div>
    </div>
  <?php
  }
