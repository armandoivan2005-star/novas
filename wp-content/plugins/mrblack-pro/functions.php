...
if( !function_exists('mrblack_pro_get_template_plugin_part') ) {
    function mrblack_pro_get_template_plugin_part( $file_path, $module, $template, $slug ) {

        $html             = '';
        $template_path    = MRBLACK_PRO_DIR_PATH . 'modules/' . esc_attr($module);
    ...
    add_filter( 'mrblack_get_template_plugin_part', 'mrblack_pro_get_template_plugin_part', 20, 4 );
}

if( !function_exists('mrblack_pro_before_after_widget') ) {
    function mrblack_pro_before_after_widget ( $content ) {
        $allowed_html = array(
            'aside' => array(
                'id'    => array(),
                'class' => array()
...
if( !function_exists('mrblack_pro_widget_title') ) {
    function mrblack_pro_widget_title( $content ) {
...
if( !function_exists('mrblack_custom_post_type_elementor_support') ) {
    function mrblack_custom_post_type_elementor_support() {
...
add_action('init', 'mrblack_custom_post_type_elementor_support');
