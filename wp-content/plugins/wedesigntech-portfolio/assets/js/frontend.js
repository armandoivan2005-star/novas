var wdtPortfolioFrontendUtils = {
	...
	wdtPortfolioImageHoverContent: function () {
		...
		jQuery('.wdt-listings-item-wrapper').mousemove(function (event) {
			...
			const $hoverSection = $wrapper.find('.wdt-listings-item-hover-section');
		  
			const x = event.offsetX;
			const y = event.offsetY;
		  
			$hoverSection.css('transform', 'translate3d(' + x + 'px, ' + y + 'px, 0)');
		  
			const rect = $hoverSection[0].getBoundingClientRect();
			...
			if (isOutOfViewport.left) {
			  $hoverSection.css('transform', 'translate3d(calc(' + x + 'px + 100%), ' + y + 'px, 0)');
			} else if (isOutOfViewport.right) {
			  $hoverSection.css('transform', 'translate3d(calc(' + x + 'px - 100%), ' + y + 'px, 0)');
			}
		  });
	},
