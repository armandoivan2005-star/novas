...
function wdt_generate_listing_output_loop($filtered_item_ids, $output_options) {
	...
	$enable_carousel = isset($output_options['enable_carousel']) ? $output_options['enable_carousel'] : false;
					...
					if(is_array($wdt_media_images_ids) && !empty($wdt_media_images_ids)) {
						foreach($wdt_media_images_ids as $wdt_media_attachments_id) {
							...
							if (is_array($thumbnail_url)) {
								...
								error_log("Failed to get image for attachment ID: " . $wdt_media_attachments_id);
							}
						}
					}
				}
...
if(!function_exists('breadcrumbs_portfolio_module')) {
    ...
    add_filter( 'mrblack_breadcrumbs', 'breadcrumbs_portfolio_module', 10, 1 );
}
