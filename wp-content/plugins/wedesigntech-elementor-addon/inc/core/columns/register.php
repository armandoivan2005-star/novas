...
if (! class_exists ( 'WeDesignTechElementorColumn' )) {
    class WeDesignTechElementorColumn {
        ...
        public function __construct() {
            ...
            add_action( 'wp_head', array( $this, 'trigger_column_render' ) );
        }

        public function trigger_column_render() {
             $post_id = get_the_ID();
                if ( $post_id && \Elementor\Plugin::$instance->documents->get( $post_id ) ) {
                    ...
                    if (isset($elementor_data['elements'])) {
                    $sticky_column_found = false;
                    foreach ( $elementor_data as $element ) {
                        if ( isset( $element['elements'] ) && is_array( $element['elements'] ) ) {
                            foreach ( $element['elements'] as $sub_element ) {
                                if ( $sub_element['elType'] === 'column' && !empty( $sub_element['settings']['wdt_sticky_column'] ) ) {
                                    $sticky_column_found = true;
                                    break 2;
                                }
                            }
                        }
                    }
                    if ( $sticky_column_found ) {
                        update_option('wdt_sticky_script', 'yes', true);
                    } else {
                        update_option('wdt_sticky_script', 'no', true);
                    }
                } else {
                }
            }
        ...
        public function enqueue_scripts() {
            $wdt_sticky_script=get_option( 'wdt_sticky_script', true);
                if($wdt_sticky_script=="yes") {
