...
class WeDesignTech_Widget_Base_Advanced_Toggle {
    ...
    public function create_elementor_controls($elementor_object) {
		...
		$elementor_object->add_control( 'offer_range', array(
			'label'       => esc_html__( 'Offer Range', 'wdt-elementor-addon' ),
			'type'        => \Elementor\Controls_Manager::TEXT,
			'label_block' => true,
			'placeholder' => esc_html__( 'Your offer range goes here', 'wdt-elementor-addon' ),
			'default'     => esc_html__( '20% OFF', 'wdt-elementor-addon' )
		) );
    ...
    public function render_html($widget_object, $settings){
		...
		$output .= '<div class="wdt-advanced-toggle-container">';
			$output .= '<div class="wdt-advanced-toggle-switch-wrapper">';

				$output .= '<div class="wdt-advanced-toggle-content-container">';
					if (isset($settings['title'])) {
						$output .= $this->cc_repeater_contents->render_title($settings['title'], $link_start = '', $link_end = '');
					}
					if (isset($settings['sub_title'])) {
				...
				$output .= '<div class="wdt-advanced-toggle-switcher-container">';
					...
					if( isset($settings['right_side_title']) && !empty($settings['right_side_title'])) {
						...
						$output .= '</div>';
						if (isset($settings['offer_range']) && !empty($settings['offer_range'])) {
							$output .= '<span class="wdt-offer-price">' . $settings['offer_range'] . '</span>';
						}
					}
					$output .= '</div>';
				$output .= '</div>';
