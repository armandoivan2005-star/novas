...
class WeDesignTech_Widget_Base_Pricing_Table {
	...
	public function create_elementor_controls($elementor_object) {
		...
		// Header
			...
			$elementor_object->start_controls_section( 'wdt_section_header', array(
				...
				$elementor_object->add_control( 'header_description', array(
				...
				$header_positions->add_control( 'element_value', array(
					...
					'options' => array(
						'title' => esc_html__( 'Title', 'wdt-elementor-addon'),
						'subtitle' => esc_html__( 'Sub Title', 'wdt-elementor-addon'),
						'header_description' => esc_html__( 'Header Description', 'wdt-elementor-addon')
					)
				) );
				$elementor_object->add_control( 'header_positions', array(
					...
					'default'       =>  array(
						...
						array(
							'element_value' => 'header_description'
						),
					),
					'prevent_empty' => true,
		...
		// Pricing
			...
			$elementor_object->start_controls_section( 'wdt_section_pricing', array(
				...
				$elementor_object->add_control( 'description', array(
					'label'       => esc_html__( 'Description', 'wdt-elementor-addon' ),
					'type'        => \Elementor\Controls_Manager::TEXT,
					'label_block' => true,
					'placeholder' => esc_html__( 'Your description goes here', 'wdt-elementor-addon' ),
		...
		// Features
			...
			$elementor_object->start_controls_section( 'wdt_section_features', array(
				...
				$elementor_object->add_control( 'features_content_title', array(
					'type'        => \Elementor\Controls_Manager::TEXT,
					'label'       => esc_html__('Features Title', 'wdt-elementor-addon'),
					'description' => esc_html__('Enter the title for the Features section.', 'wdt-elementor-addon'),
					'default'     => esc_html__('Our Features', 'wdt-elementor-addon'),
				) );
		...
		if(is_array($header_content_positions) && !empty($header_content_positions)) {
			...
			foreach($header_content_positions as $content_position) {
				if($content_position == 'title') {
					...
					$output .= $this->cc_repeater_contents->render_sub_title($settings['sub_title']);
				} else if($content_position == 'header_description') {
					$output .= $this->cc_repeater_contents->render_description($settings['header_description']);
				}
			}
			$output .= '</div>';
		}
	...
	public function render_pricing_inner_html($settings) {
		...
		$output .= '</div>';
		$output .= '<div class="wdt-pricing-table-pricing-description">';
				$output .= $this->cc_repeater_contents->render_description($settings['description']);
		$output .= '</div>';
	...
	public function render_features_html($settings, $widget_object) {
		...
		if (!empty($settings['features_content_title'])) {
			$output .= '<div class="wdt-pricing-table-features-title">';
				$output .= '<h3>' . esc_html($settings['features_content_title']) . '</h3>';
			$output .= '</div>';
		}
