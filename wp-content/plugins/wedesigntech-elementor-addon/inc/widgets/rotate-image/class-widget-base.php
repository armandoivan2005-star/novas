...
class WeDesignTech_Widget_Base_Rotate_Image {
	...
	public function create_elementor_controls($elementor_object) {
		...
		$elementor_object->add_control(
			'rotate_image_txt',
			array (
				'label' => esc_html__( 'Link 1', 'wdt-elementor-addon' ),
		...
		$elementor_object->add_control(
			'rotate_image_txt1',
			array (
				'label' => esc_html__( 'Link 2', 'wdt-elementor-addon' ),
				'type' => \Elementor\Controls_Manager::TEXT,
				'default' => '#',
			)
		);
	...
	public function render_html($widget_object, $settings) {
		...
		$output .= '<div class="wdt-rotate-image-container" data-settings="'.esc_js(wp_json_encode($animation_settings)).'">';
			$output .= '<div class="wdt-rotate-image" id="wdt-rotate-image" >';
			if( (isset($settings['rotate_image']['url']) && !empty($settings['rotate_image']['url'])) ) {
				...
				$output .= '<a href="'.esc_url( $settings['rotate_image_txt'] ).'" class="wdt-rotate-link" >';
					$output .= \Elementor\Group_Control_Image_Size::get_attachment_image_html( $image_setting );
				$output .= '</a>';
			}
			$output .= '</div>';

			$output .= '<div class="wdt-rotate-second-image" id="wdt-rotate-second-image" >';
			if( (isset($settings['rotate_second_image']['url']) && !empty($settings['rotate_second_image']['url'])) ) {
				...
				$output .= '<a href="'.esc_url( $settings['rotate_image_txt1'] ).'" class="wdt-rotate-link" >';
					$output .= \Elementor\Group_Control_Image_Size::get_attachment_image_html( $image_setting );
				$output .= '</a>';
			}
			$output .= '</div>';
