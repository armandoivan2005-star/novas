<?php
add_action( 'mrblack_after_main_css', 'notfound_style' );
function notfound_style() {
    if( is_404() ) {
        wp_enqueue_style( 'mrblack-404', get_theme_file_uri('/modules/404/assets/css/404.css'), false, MRBLACK_THEME_VERSION, 'all');
    }
}

add_filter( 'mrblack_add_inline_style', 'notfound_inline_style' );
function notfound_inline_style( $style ) {
    if( is_404() ) {
        $params = mrblack_404_page_params();
...
function mrblack_404_page_params() {
    $params = array(
        'enable_404message' => 1,
    ...
    return apply_filters( 'mrblack_404_page_params', $params );
}