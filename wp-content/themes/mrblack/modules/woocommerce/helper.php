...
if( !class_exists( 'MrBlack_Woo_Helper' ) ) {

    class MrBlack_Woo_Helper {
        ...
        function __construct() {
			...
			// Utils
				...
				include_once MRBLACK_MODULE_DIR . '/woocommerce/utils.php';


			// After theme setup

				add_action( 'mrblack_after_load_module_helpers', array ( $this, 'load_woo_module_support' ), 10 );
			...
			// Bredcrumbs
				...
				// Bredcrumb Title
					...
					add_filter( 'mrblack_breadcrumb_title', array ( $this, 'breadcrumb_title_woo_module' ), 10, 1 );

				// Bredcrumb Breadcrumbs

					add_filter( 'mrblack_breadcrumbs', array ( $this, 'breadcrumbs_woo_module' ), 10, 1 );
			...
			// Enqueue Files
				...
				// Enqueue CSS Files
					...
					add_action( 'mrblack_after_main_css', array ( $this, 'woo_after_main_css' ) );

				// Enqueue JS Files

					add_action( 'mrblack_after_enqueue_js', array ( $this, 'woo_after_main_js' ) );
			...
			function woo_output_content_wrapper() {
				...
				<!-- ** Primary ** -->
                    ...
                    if( is_shop() || is_product_category() || is_product_tag() ) {
                        do_action( 'mrblack_woo_before_products_loop' );
                    }
		...
		*/
			function woo_output_content_wrapper_end() {
					...
					if( is_shop() || is_product_category() || is_product_tag() ) {
						do_action( 'mrblack_woo_after_products_loop' );
					}
				...
				<?php
				mrblack_template_part( 'sidebar', 'templates/sidebar' );
			...
			function breadcrumb_title_woo_module( $title ) {
				...
				if( is_shop() ) {
					...
					if( get_option('woocommerce_shop_page_id') == '' ) {
						$title = '<h1>'.esc_html__('Shop', 'mrblack').'</h1>';
					} else {
		...
		*/
			function breadcrumbs_woo_module( $breadcrumbs ) {
				...
				if( $shop_page_id = get_option('woocommerce_shop_page_id') ) {
					...
					$shop_page_title = esc_html__('Shop', 'mrblack');
					$shop_page_link  = '#';
				}
		...
		*/
			function woocommerce_locate_template( $template, $template_name, $template_path ) {
				...
				$plugin_path  = MRBLACK_MODULE_DIR . '/woocommerce/templates/';
		...
		*/
			function woo_after_main_css() {
				...
				/* Before Hook */
					...
					do_action( 'mrblack_before_woo_css' );

					wp_register_style( 'mrblack-woo', MRBLACK_MODULE_URI . '/woocommerce/assets/css/default.css', array (), MRBLACK_THEME_VERSION, 'all' );
					wp_enqueue_style( 'mrblack-woo' );
				...
				/* Main CSS */
					...
					if( is_shop() || is_product_category() || is_product_tag() || is_product() || is_cart() || is_checkout() || is_account_page() || ( isset( $post->ID ) && is_page('wishlist') ) ) {
						...
						/* General CSS */
							...
							// Carousel Styles
								$css_file_path = MRBLACK_MODULE_DIR . '/woocommerce/assets/css/carousel.css';
							...
							// Filter for inline styles
								$css = apply_filters( 'mrblack_woo_css', $css );

							if( !empty($css) ) {
								wp_add_inline_style( 'mrblack-woo', $css );
							}
						...
						/* Archive CSS */
							...
							wp_register_style( 'mrblack-woo-archive', '', array (), MRBLACK_THEME_VERSION, 'all' );
							wp_enqueue_style( 'mrblack-woo-archive' );

							// Filter for inline styles
								$css = apply_filters( 'mrblack_woo_archive_css', '' );

							if( !empty($css) ) {
								wp_add_inline_style( 'mrblack-woo-archive', $css );
							}
				...
				/* Others CSS */
					if( is_cart() || is_checkout() || is_account_page() || ( isset( $post->ID ) && is_page('wishlist') ) ) {
						...
						/* General CSS */
							...
							wp_register_style( 'mrblack-woo-others', '', array (), MRBLACK_THEME_VERSION, 'all' );
							wp_enqueue_style( 'mrblack-woo-others' );
							...
							if( is_cart() || ( isset( $post->ID ) && is_page('wishlist') ) ) {
								...
								$css_file_path = MRBLACK_MODULE_DIR . '/woocommerce/assets/css/cart.css';
							...
							if( is_checkout() ) {
								...
								$css_file_path = MRBLACK_MODULE_DIR . '/woocommerce/assets/css/checkout.css';
							...
							if( is_account_page() ) {
								...
								$css_file_path = MRBLACK_MODULE_DIR . '/woocommerce/assets/css/myaccount.css';
							...
							if( ( isset( $post->ID ) && is_page('wishlist') ) ) {
								...
								$css_file_path = MRBLACK_MODULE_DIR . '/woocommerce/assets/css/wishlist.css';
							...
							// Filter for inline styles
								$css = apply_filters( 'mrblack_woo_others_css', $css );

							if( !empty($css) ) {
								wp_add_inline_style( 'mrblack-woo-others', $css );
							}
				...
				/* After Hook */
					...
					do_action( 'mrblack_after_woo_css' );
		...
		*/
			function woo_after_main_js() {
				...
				/* Before Hook */
					...
					do_action( 'mrblack_before_woo_js' );
				...
				/* Main JS */
					...
					if( is_shop() || is_product_category() || is_product_tag() || is_product() || is_cart() || is_checkout() ) {
						...
						/* General JS */
							...
							wp_register_script( 'mrblack-woo', '', array ('jquery'), false, true );
							wp_enqueue_script( 'mrblack-woo' );

							// JS Variables
								$woo_objects = apply_filters( 'mrblack_woo_objects', array (
									'ajaxurl' => esc_url( admin_url('admin-ajax.php') )
								) );
								wp_localize_script('mrblack-woo', 'wdtShopObjects', $woo_objects);
							...
							// Inline JS Scripts
								$js = apply_filters( 'mrblack_woo_js', $js );

								if( !empty($js) ) {
									wp_add_inline_script( 'mrblack-woo', $js );
								}


						/* Archive JS */

							wp_register_script( 'mrblack-woo-archive', '', array ('jquery'), false, true );
							wp_enqueue_script( 'mrblack-woo-archive' );

							$js = apply_filters( 'mrblack_woo_archive_js', '' );

							if( !empty($js) ) {
								wp_add_inline_script( 'mrblack-woo-archive', $js );
							}
				...
				/* After Hook */
					...
					do_action( 'mrblack_after_woo_js' );
		...
		*/
			function woo_load_modules() {
				...
				/* Load Listing Helpers */
					...
					include_once MRBLACK_MODULE_DIR. '/woocommerce/listings/index.php';


				/* Template Pages */

					include_once MRBLACK_MODULE_DIR. '/woocommerce/shop/index.php';
					include_once MRBLACK_MODULE_DIR. '/woocommerce/category/index.php';
					include_once MRBLACK_MODULE_DIR. '/woocommerce/tag/index.php';

				/* Product Single */

					include_once MRBLACK_MODULE_DIR. '/woocommerce/single/index.php';

				/* Others */

					include_once MRBLACK_MODULE_DIR. '/woocommerce/others/index.php';
		...
		*/
			...
			function woo_wp_footer() {
				...
				preg_match_all('#<style id=\'mrblack-woo-non-archive-inline-css\' type=\'text/css\'>(.*?)</style>#is', $content, $matches, PREG_SET_ORDER);
...
if( !function_exists('mrblack_woo_helper') ) {
	function mrblack_woo_helper() {
		if ( class_exists( 'WooCommerce' ) ) {
			return MrBlack_Woo_Helper::instance();
		}
	}
}
