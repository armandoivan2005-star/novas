...
if( !function_exists('mrblack_archive_blog_post_defaults') ) {
    function mrblack_archive_blog_post_defaults() {
        $defaults = array(
            'post-layout'      => 'entry-grid',
            'post-cover-style' => 'wdt-classic',
            'post-gl-style'    => 'wdt-minimal',
...
if( !function_exists('mrblack_archive_blog_post_misc_defaults') ) {
    function mrblack_archive_blog_post_misc_defaults() {
        $defaults = array(
...
if( !function_exists('mrblack_archive_blog_post_params_default') ) {
    function mrblack_archive_blog_post_params_default() {
        $params = array(
            'enable_post_format'   	 => 0,
            'enable_video_audio' 	 => 0,
            'enable_gallery_slider'  => 1,
            'archive_post_elements'  => array( 'feature_image', 'meta_group', 'title', 'read_more' ),
            'archive_meta_elements'  => array( 'date' ),
            'archive_readmore_text'  => esc_html__('Read More', 'mrblack'),
            'enable_excerpt_text'	 => 0,
...
if( !function_exists('mrblack_archive_blog_post_defaults_filter') ) {
    function mrblack_archive_blog_post_defaults_filter() {
        $defaults = mrblack_archive_blog_post_defaults();
        return apply_filters( 'mrblack_archive_post_cmb_class', $defaults );
    }
}

if( !function_exists('mrblack_archive_blog_post_misc_defaults_filter') ) {
    function mrblack_archive_blog_post_misc_defaults_filter() {
        $defaults = mrblack_archive_blog_post_misc_defaults();
        return apply_filters( 'mrblack_archive_post_hld_class', $defaults );
    }
}

if( !function_exists('mrblack_archive_blog_post_params') ) {
    function mrblack_archive_blog_post_params() {
        $params = mrblack_archive_blog_post_params_default();
        return apply_filters( 'mrblack_archive_blog_post_params', $params );
    }
}

if( !function_exists('mrblack_get_archive_post_combine_class') ) {
	function mrblack_get_archive_post_combine_class() {

        $blog_defaults = mrblack_archive_blog_post_defaults_filter();
        ...
        return apply_filters( 'mrblack_get_archive_post_combine_class', implode( ' ', $combine_class ) );
	}
}

if( !function_exists('mrblack_get_archive_post_holder_class') ) {
	function mrblack_get_archive_post_holder_class() {

        $blog_defaults = mrblack_archive_blog_post_defaults_filter();
        $blog_misc_defaults = mrblack_archive_blog_post_misc_defaults_filter();
        ...
        return apply_filters( 'mrblack_get_archive_post_holder_class', implode( ' ', $holder_class ) );
	}
}

if( !function_exists('mrblack_get_archive_post_style') ) {
	function mrblack_get_archive_post_style() {
		...
		return apply_filters( 'mrblack_get_archive_post_style', $post_style );
	}
}

if( !function_exists('mrblack_get_archive_post_column') ) {
	function mrblack_get_archive_post_column() {
		...
		return apply_filters( 'mrblack_get_archive_post_column', $post_columns );
	}
}

add_filter('post_class', 'mrblack_archive_set_post_class', 10, 3);
if( !function_exists('mrblack_archive_set_post_class') ) {
    function mrblack_archive_set_post_class( $classes, $class, $post_id ) {
        ...
        if( is_post_type_archive('post') || is_search() || is_category() || is_tag() || is_home() || is_author() || is_year() || is_month() || is_day() || is_time() || is_tax('post_format') || ( defined('DOING_AJAX') && DOING_AJAX ) ) {
            ...
            $blog_params = mrblack_archive_blog_post_params();
...
add_action( 'mrblack_after_main_css', 'mrblack_blog_enqueue_css', 10 );
if( !function_exists( 'mrblack_blog_enqueue_css' ) ) {
	function mrblack_blog_enqueue_css() {
		wp_enqueue_style( 'wdt-blog', get_theme_file_uri('/modules/blog/assets/css/blog.css'), false, MRBLACK_THEME_VERSION, 'all');
		...
		wp_enqueue_style( 'jquery-bxslider', get_theme_file_uri('/modules/blog/assets/css/jquery.bxslider.css'), false, MRBLACK_THEME_VERSION, 'all' );
	}
}

add_action( 'mrblack_before_enqueue_js', 'mrblack_blog_enqueue_js' );
if( !function_exists( 'mrblack_blog_enqueue_js' ) ) {
	function mrblack_blog_enqueue_js() {
...
if( !function_exists( 'after_blog_post_content_pagination' ) ) {
    function after_blog_post_content_pagination() {
    	...
    	$pagination_template = mrblack_archive_blog_post_params();
    	$pagination_template = $pagination_template['archive_blog_pagination'];

        echo apply_filters( 'mrblack_blog_archive_pagination', mrblack_get_template_part( 'pagination', 'templates/'.$pagination_template ) );
    }
    add_action( 'mrblack_after_blog_post_content_wrap', 'after_blog_post_content_pagination' );
}

if( !function_exists( 'mrblack_excerpt' ) ) {
	function mrblack_excerpt( $limit = NULL ) {
